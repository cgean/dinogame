...
export default class Character extends MotionColorFigure {
    ...
    draw() {
        for (let i = 0; i < this.figuresFires.length; i++) {
            this.figuresFires[i].draw();
        }
        const ind = Math.floor(this.imageFrameIndex) % this.images.length;
        this.ctx.drawImage(this.images[ind], this.x, this.y, this.width, this.height);
    }
    ...
    fire() {
        if (this.fires > 0) {
            let img = new MotionImageFigure(this.ctx, this.x + this.width / 2, 
                this.y + 50, 40, 30, -5, 'img/fire-ball.png');
            this.figuresFires.push(img);
            this.fires--;
        }
    }
    ...
    update(figures) {
        ...
        this.updateY();
        this.updateX(figures);
        this.checkCollision(figures);
    }
    ...
    updateX(figures) { 
        ...
        this.updateFiguresX(figures);  
    }

    updateFiguresX(figures) {
        figures = figures.filter((fig) => {
            return fig.x <= this.ctxWidth;
          });
        for (let i = 0; i < this.figuresFires.length; i++) {
            this.checkCollisionFire(this.figuresFires[i], figures.filter(f => f instanceof MotionImageFigure));
            this.figuresFires[i].x += 3;    
        }        
    }

    updateFiguresY() {
        for(let i = 0; i < this.figuresFires.length; i++) {
            if (this.figuresFires[i].y < this.ctxHeight - 60)  {
                this.figuresFires[i].y += 1;
            }
        }
    }
